...
const panelSchema = `CREATE TABLE IF NOT EXISTS panel_media (
 ...
 CREATE TABLE IF NOT EXISTS panel_resync (instance_id TEXT PRIMARY KEY,requested_at INTEGER NOT NULL);` + panelEditSchema + panelDeleteSchema
...
func (h *Handlers) uzPanelHistory(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = h.mgr.store.db.Exec(`DELETE FROM panel_delete_requests WHERE created_at<?`, time.Now().Add(-48*time.Hour).UnixMilli())
